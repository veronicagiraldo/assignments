...
webRouter.route('/:listId')
  .get((req, res, next) => {
    List.findOne({_id: req.params.listId, user: req.user._id}, (err, list) => {
      if(err){
        res.status(500);
        return next(err);
      };
  ...
  .delete((req, res, next) => {
    List.findOneAndRemove({_id: req.params.listId, user: req.user._id}, (err, todo) => {
      if (err){
        res.status(500);
        return next(err);
      }
